...
class BoxRecord:
    """One user-inserted text box: durable id + its current placement."""
...
def write_boxes(doc: pymupdf.Document, boxes: list[BoxRecord]) -> None:
    ...
    payload = json.dumps([{"id": b.id, "page": b.page, "rect": list(b.rect)} for b in boxes])
    doc.xref_set_key(doc.pdf_catalog(), _PRIVATE_KEY, pymupdf.get_pdf_str(payload))


def add_box(doc: pymupdf.Document, page: int, rect: tuple[float, float, float, float]) -> BoxRecord:
    """Register a new box; returns its record (fresh unique id)."""
    box = BoxRecord(id=uuid.uuid4().hex[:12], page=page, rect=tuple(rect))
...
def update_box_rect(
    ...
    """Move/resize a registered box (unknown ids are ignored)."""
    rect = tuple(rect)
    write_boxes(
        doc,
        [BoxRecord(b.id, b.page, rect) if b.id == box_id else b for b in read_boxes(doc)],
    )
...
def remap_pages(doc: pymupdf.Document, mapping: dict[int, int]) -> None:
    ...
    write_boxes(
        doc,
        [BoxRecord(b.id, mapping[b.page], b.rect) for b in boxes if b.page in mapping],
    )
