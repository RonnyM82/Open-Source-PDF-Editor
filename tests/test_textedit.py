...
# --- list creation from scratch (L3) -----------------------------------------


def _blank_pdf(tmp_path, name="blank.pdf"):
    doc = pymupdf.open()
...
def _insert_items(doc, texts, kind, point=(72.0, 200.0)):
    """Create a list by chaining ``next_point``, the way the UI's Enter
    continuation does. Returns the results and the box boundaries they imply."""
    results, bounds = [], []
    for i, text in enumerate(texts):
        result = doc.insert_list_item(0, point, [StyledRun(text, TextStyle())], kind, ordinal=i + 1)
        results.append(result)
        bounds.append((result.bbox, "\n".join(result.visual_lines)))
        point = result.next_point
    return results, bounds


def test_insert_list_item_creates_a_hanging_bullet(tmp_path):
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        result = doc.insert_list_item(
            0, (72.0, 200.0), [StyledRun("A brand new item", TextStyle())], "bullet"
        )
        assert result.kind == "bullet"
        out = tmp_path / "one.pdf"
        doc.save(out)
    with PdfDocument.open(out) as doc:  # round-trip (rule 10)
        item = doc.paragraphs(0)[0]
        assert item.hang_indent > 0
        assert item.text.lstrip()[:1] in ("•", "·")
        assert "A brand new item" in item.text
        assert list_item_kind(item) == ("bullet", 1)


def test_insert_list_item_fingerprint_carries_the_marker(tmp_path):
    """The registry fingerprint must be what the page RE-EXTRACTS. A bullet is
    drawn as its own span and folded onto the front of the body line, so a
    fingerprint built from the laid-out body lines would own nothing — and the
    next item (which sits closer than MuPDF's block split) would merge in."""
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        result = doc.insert_list_item(
            0, (72.0, 200.0), [StyledRun("Fingerprint me", TextStyle())], "bullet"
        )
        assert result.visual_lines
        assert result.visual_lines[0][:1] in ("•", "·")
        assert "Fingerprint me" in result.visual_lines[0]


def test_insert_list_item_fingerprint_excludes_a_same_baseline_neighbour(tmp_path):
    """The fingerprint is restricted to the lines the op DREW. The item's box
    is marker-widened, so a value in the next column can sit inside it and
    share a baseline — absorbing it would make the box own foreign text, which
    a later edit then physically relocates into the box."""
    path = _blank_pdf(tmp_path, "neighbour.pdf")
    doc = pymupdf.open(str(path))
    doc[0].insert_text((300, 200), "99.00", fontsize=11)
    doc.save(str(tmp_path / "n2.pdf"))
    doc.close()
    with PdfDocument.open(tmp_path / "n2.pdf") as pdoc:
        result = pdoc.insert_list_item(
            0, (72.0, 200.0), [StyledRun("Description of the item", TextStyle())], "bullet"
        )
        assert result.bbox[2] < 300  # the drawn item stops short of the value
        assert all("99.00" not in line for line in result.visual_lines)


def test_insert_list_item_continues_as_separate_items(tmp_path):
    """Three chained items stay three editable items — the isolation the box
    registry provides (they sit one line pitch apart, well inside MuPDF's
    dict-block split threshold, so without it they read as ONE item)."""
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        results, bounds = _insert_items(doc, ["Alpha", "Bravo", "Charlie"], "bullet")
        out = tmp_path / "three.pdf"
        doc.save(out)
    # Baselines step by exactly one pitch, markers line up.
    assert results[1].next_point[1] - results[0].next_point[1] == pytest.approx(
        results[0].next_point[1] - 200.0, abs=0.01
    )
    assert {round(r.bbox[0], 3) for r in results} == {72.0}
    with PdfDocument.open(out) as doc:
        items = doc.paragraphs(0, boundaries=bounds)
        assert [i.text.lstrip(" •·") for i in items] == ["Alpha", "Bravo", "Charlie"]
        assert all(i.hang_indent > 0 for i in items)


def test_insert_list_item_numbers_in_order(tmp_path):
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
...
def test_insert_list_item_wraps_long_text(tmp_path):
    """The body wraps to the page margin. It used to be refused outright — and
    the refusal discarded what the user had typed and ended the list."""
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        result = doc.insert_list_item(
            0, (72.0, 200.0), [StyledRun("word " * 40, TextStyle())], "bullet"
        )
        assert len(result.visual_lines) > 1
        assert result.bbox[2] < 595  # stays on the page
        # Continuation clears the whole wrapped item, not just its first line.
        assert result.next_point[1] > result.bbox[3] - 5


def test_insert_list_item_keeps_typed_marker_like_text(tmp_path):
    """Freshly TYPED text is content, not a marker to re-format: stripping a
    leading ordinal silently deleted the "a) " from "a) see appendix"."""
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        result = doc.insert_list_item(
            0, (72.0, 200.0), [StyledRun("a) see appendix", TextStyle())], "bullet"
        )
    assert "a) see appendix" in result.visual_lines[0]


def test_insert_list_item_refuses_bad_input_without_mutating(tmp_path):
    """Every refusal is PRE-mutation — the earlier insert-then-restyle shape
    left the plain text on the page when its second half raised."""
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        with pytest.raises(ValueError, match="list kind"):
            doc.insert_list_item(0, (72.0, 200.0), [StyledRun("x", TextStyle())], "dashed")
        with pytest.raises(ValueError, match="no text"):
            doc.insert_list_item(0, (72.0, 200.0), [StyledRun("   ", TextStyle())], "bullet")
        with pytest.raises(ValueError, match="outside the page"):
            doc.insert_list_item(0, (72.0, 9999.0), [StyledRun("x", TextStyle())], "bullet")
        # Within one ascent of the page top there is no room for the line.
        with pytest.raises(ValueError, match="does not fit"):
            doc.insert_list_item(0, (72.0, 5.0), [StyledRun("Top", TextStyle())], "bullet")
        # Hard against the right margin there is no room for the body.
        with pytest.raises(ValueError, match="no room"):
            doc.insert_list_item(0, (590.0, 200.0), [StyledRun("x", TextStyle())], "bullet")
        assert not doc._doc[0].get_text().strip()  # nothing landed


def test_insert_list_item_leaves_existing_text_alone(tmp_path):
    """Additive, like every other insert. The earlier insert-then-restyle shape
    hit-tested the new text's position and converted whatever paragraph came
    back — so an item landing near existing prose bulleted THAT and shoved its
    body 18 pt right, leaving the typed item plain."""
    path = _blank_pdf(tmp_path, "occupied.pdf")
    doc = pymupdf.open(str(path))
    doc[0].insert_text((72, 213.2), "Existing paragraph line one", fontsize=11)
    doc[0].insert_text((72, 226.4), "existing line two", fontsize=11)
    doc.save(str(tmp_path / "o2.pdf"))
    doc.close()
    with PdfDocument.open(tmp_path / "o2.pdf") as pdoc:
        first = pdoc.insert_list_item(0, (72.0, 200.0), [StyledRun("Alpha", TextStyle())], "bullet")
        pdoc.insert_list_item(0, first.next_point, [StyledRun("Bravo", TextStyle())], "bullet")
        spans = {s.text: s.origin for s in pdoc.text_spans(0)}
    # The pre-existing lines keep their own x — neither moved into a hang.
    assert spans["Existing paragraph line one"][0] == pytest.approx(72.0, abs=0.5)
    assert spans["existing line two"][0] == pytest.approx(72.0, abs=0.5)


def test_slot_is_occupied_sees_foreign_text_but_not_the_caller_s_own(tmp_path):
    path = _blank_pdf(tmp_path, "slot.pdf")
    doc = pymupdf.open(str(path))
    doc[0].insert_text((72, 303), "In the way", fontsize=11)  # ~one pitch down
    doc.save(str(tmp_path / "s2.pdf"))
    doc.close()
    with PdfDocument.open(tmp_path / "s2.pdf") as pdoc:
        result = pdoc.insert_list_item(0, (72.0, 286.8), [StyledRun("Item", TextStyle())], "bullet")
        # The next slot lands on the existing line -> the list must stop there.
        assert pdoc.slot_is_occupied(0, result.next_bbox, (result.bbox,))
        # The item's OWN box overlaps its successor's slot by a couple of points
        # (box height is 1.45x the size against a 1.2x pitch) — ignoring it is
        # what stops EVERY continuation reading as blocked.
        assert not pdoc.slot_is_occupied(0, result.bbox, (result.bbox,))


def test_next_list_item_fits_stops_at_the_page_bottom(tmp_path):
    with PdfDocument.open(_blank_pdf(tmp_path)) as doc:
        assert doc.next_list_item_fits(0, (72.0, 800.0))
        assert not doc.next_list_item_fits(0, (72.0, 842.0))
