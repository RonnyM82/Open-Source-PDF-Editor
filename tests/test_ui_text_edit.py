...
def _enter(view, mods=Qt.KeyboardModifier.NoModifier):
    """Press Enter in the open paragraph editor (no synthetic mouse anywhere in
    this suite — key events go straight to the widget)."""
    view._para_editor.keyPressEvent(QKeyEvent(QEvent.Type.KeyPress, Qt.Key.Key_Return, mods))


def _start_list(window, view, x=80.0, y=200.0):
    window.insert_list()
    z = view._canvas.render_zoom
    view._canvas.insertPointSelected.emit(x * z, y * z)


def _type_items(view, texts, last=None):
    """Type each item and press Enter after it; ``last`` overrides the final
    key's modifiers (Ctrl to end the list there)."""
    for i, text in enumerate(texts):
        view._para_editor.textCursor().insertText(text)
        final = last is not None and i == len(texts) - 1
        _enter(view, last if final else Qt.KeyboardModifier.NoModifier)


def _items(view):
    """The page's list items as the UI sees them — through the geometry cache,
    which passes the box-registry boundaries that keep them separate."""
    return [p for p in view.page_geometry(0).paragraphs if p.hang_indent > 0]


def test_ui_insert_list_arms_and_continues(qapp, tmp_path):
    """Insert list arms click-to-place; the click opens the editor in LIST
    mode, and plain Enter commits each item and starts the next."""
    window = MainWindow()
    try:
        ...
        assert window._insert_list_action.isChecked()
        assert view._canvas._insert_armed

        z = view._canvas.render_zoom
        view._canvas.insertPointSelected.emit(80 * z, 200 * z)
        assert not view._para_editor.isHidden()
        assert view._para_editor.list_mode

        _type_items(view, ["Alpha", "Bravo", "Charlie"])
        assert view.undo_stack.count() == 3  # one undoable command per item
        assert not view._para_editor.isHidden()  # still asking for a 4th item
...
def test_ui_insert_list_items_own_separate_boxes(qapp, tmp_path):
    """Each item registers its OWN box with the fingerprint the page really
    re-extracts (the marker included). Built from the reported body lines the
    boxes would own nothing and the items — one line pitch apart, inside
    MuPDF's block-split threshold — would collapse into one hanging item."""
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["Alpha", "Bravo", "Charlie"])
        view._para_editor.cancel()

        boxes = view.document.boxes(0)
        assert len(boxes) == 3
        assert all(b.text.lstrip()[:1] in ("•", "·") for b in boxes)
        assert len(_items(view)) == 3
    finally:
        window.close()


def test_ui_insert_list_empty_item_ends_the_list(qapp, tmp_path):
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["Only one"])
        assert view.undo_stack.count() == 1

        _enter(view)  # Enter on the empty next item
        assert view._para_editor.isHidden()
        assert view.undo_stack.count() == 1  # nothing inserted for the empty item
        assert view._pending_list is None
    finally:
        window.close()


def test_ui_insert_list_ctrl_enter_ends_the_list(qapp, tmp_path):
    """Ctrl+Enter keeps its app-wide 'apply' meaning: the item commits and the
    list stops, rather than asking for another one."""
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["Alpha", "Bravo"], last=Qt.KeyboardModifier.ControlModifier)
        assert view.undo_stack.count() == 2
        assert view._para_editor.isHidden()
        assert len(_items(view)) == 2
    finally:
        window.close()


def test_ui_insert_list_shift_enter_breaks_the_line(qapp, tmp_path):
    """Shift+Enter is the escape hatch for a two-line item — Enter now means
    'next item', so the plain line break needs its own key."""
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        view.set_edit_mode(True)
        _start_list(window, view)
        view._para_editor.textCursor().insertText("Line one")
        _enter(view, Qt.KeyboardModifier.ShiftModifier)
        assert not view._para_editor.isHidden()  # NOT committed
        assert view.undo_stack.count() == 0
        view._para_editor.textCursor().insertText("Line two")
        _enter(view, Qt.KeyboardModifier.ControlModifier)

        item = _items(view)[0]
        assert "Line one" in item.text and "Line two" in item.text
        assert len(item.lines) == 2  # one item, two visual lines
    finally:
        window.close()


def test_ui_insert_list_numbered_follows_the_toolbar_kind(qapp, tmp_path):
    window = MainWindow()
    try:
        ...
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["One", "Two", "Three"], last=Qt.KeyboardModifier.ControlModifier)
        texts = [p.text for p in view.page_geometry(0).paragraphs]
        assert texts == ["1. One", "2. Two", "3. Three"]
    finally:
        window.close()


def test_ui_insert_list_undo_unwinds_each_item(qapp, tmp_path):
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        ...
        view.undo_stack.undo()
        assert "Bravo" not in _page_text(view)
        assert "Alpha" in _page_text(view)
        view.undo_stack.undo()
        assert not _page_text(view).strip()
        assert not view.dirty
        assert not view.document.boxes(0)  # the registry unwound with the content
    finally:
        window.close()


def test_ui_plain_insert_started_mid_list_is_not_a_list_item(qapp, tmp_path):
    """Arming a different click-to-place mode ends the list. Before the reset,
    a plain Insert text started mid-list inherited the stale (kind, ordinal)
    and came out as the list's next item."""
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["Alpha"])
        assert view._pending_list is not None  # mid-list

        window.insert_text()  # switch to a PLAIN insert without ending the list
        z = view._canvas.render_zoom
        view._canvas.insertPointSelected.emit(300 * z, 600 * z)
        assert view._pending_list is None
        assert not view._para_editor.list_mode

        view._para_editor.textCursor().insertText("Plain note")
        view._para_editor.commit()
        note = next(p for p in view.page_geometry(0).paragraphs if "Plain note" in p.text)
        assert note.text == "Plain note"  # no marker prepended
        assert note.hang_indent == 0.0
    finally:
        window.close()


def test_ui_insert_list_stops_before_existing_text(qapp, tmp_path):
    """The APP picks the continuation position (the user picked only the
    first), so it stops rather than auto-placing an item onto other content."""
    import pymupdf

    path = tmp_path / "occupied.pdf"
    doc = pymupdf.open()
    page = doc.new_page(width=595, height=842)
    page.insert_text((80, 213), "Existing sentence already here", fontsize=9)
    doc.save(str(path))
    doc.close()
    ...
    try:
        ...
        view.set_edit_mode(True)
        _start_list(window, view)
        _type_items(view, ["Alpha"])  # its next slot lands on the existing line

        assert view._para_editor.isHidden()  # the list ended instead
        assert view._pending_list is None
        assert view.undo_stack.count() == 1
        span = next(s for s in view.document.text_spans(0) if "Existing sentence" in s.text)
        assert span.origin[0] == pytest.approx(80.0, abs=0.5)  # untouched
    finally:
        window.close()


def test_ui_insert_list_keeps_the_toolbar_style_between_items(qapp, tmp_path):
    """`editorClosed` restores the toolbar's GLOBAL defaults, and it fires
    after the commit handler — which, mid-list, has already re-opened the
    editor. Firing it there snapped the toolbar back while the user was still
    typing, and the next item committed with the global alignment."""
    window = MainWindow()
    try:
        ...
        window._pick_text_align("center")  # a deliberate pick while typing
        _type_items(view, ["Alpha"])
        assert not view._para_editor.isHidden()  # still typing the list
        assert window.current_text_align() == "center"  # NOT snapped back

        _type_items(view, ["Bravo"], last=Qt.KeyboardModifier.ControlModifier)
        assert view._para_editor.isHidden()
        assert all(p.align == "center" for p in _items(view))
    finally:
        window.close()


def test_ui_insert_list_is_inert_in_markup_mode(qapp, tmp_path):
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view  # Markup mode (the default)
        view.begin_insert_list("bullet")
...
def test_ui_insert_list_esc_ends_the_list(qapp, tmp_path):
    window = MainWindow()
    try:
        window.open_path(_blank_pdf(tmp_path))
        view = window.active_view
        ...
        _start_list(window, view)
        _type_items(view, ["Alpha"])
        view._para_editor.keyPressEvent(
            QKeyEvent(QEvent.Type.KeyPress, Qt.Key.Key_Escape, Qt.KeyboardModifier.NoModifier)
        )
        assert view._para_editor.isHidden()
        assert view._pending_list is None
        assert view.undo_stack.count() == 1  # the typed item stayed, nothing more
    finally:
        window.close()
